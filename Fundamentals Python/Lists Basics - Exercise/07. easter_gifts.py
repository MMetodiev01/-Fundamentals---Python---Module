...
while True:
    ...
    if order == 'OutOfStock':
        ...
        # we make variable for the index we receive if the order is 'Required'
        if index_of_gift >= 0 and index_of_gift < len(gifts_list):
            # we ask if the index is in the exact range of the list because if is not we can replace it
            gifts_list[index_of_gift] = gift
            # then in the gift list on that index we say 'replace it with the gift'
